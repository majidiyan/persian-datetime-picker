...
(function ($) {
    ...
    var persianDatepicker = (function () {
        function persianDatepicker(element, userOptions) {
            ...
            var theSelectableYears = [];
            for (var i = 1350; i <= 1415; i++)
                theSelectableYears.push(i);
            ...
            var defaults = {
                ...
                selectedDate: null,
                startDate: null,
                timePicker: false, //timepicker
                endDate: null,
                prevArrow: '\u25c4',
                nextArrow: '\u25ba',
                theme: 'default',
                alwaysShow: !1,
                selectableYears: theSelectableYears, //null,
                ...
                onShow: function () {
                    ...
                    if (self.options.timePicker) {
                        var mytop = 0;
                        try {
                            mytop = parseInt($(element).offset().top);
                        } catch (e) {
                            mytop = 0;
                            console.log(e.message);
                        }

                        $("div#ui-datepicker-div .ui-timepicker-div").css({"visibility": "visible", "top": mytop + 'px'});
                        if ($("#ui-datepicker-div").html() == '')
                            $(element).next(".ui-datepicker-trigger").trigger("click");
                    }
                    //timepicker end


                    window.global_element_datepicker = $(element);//majidian
//                    $(element).removeAttr('data-jdate');
//                    $(element).removeAttr('data-gdate');
                },
                onHide: function () {
                    ...
                    //timepicker
                    if (self.options.timePicker){
                        //show timopicker by clicking out
                        var strMonitor = $(element).closest('div').find(".monitor-persian-date").html();
                        var arrMonitor = strMonitor.split(/[ ]/);
                        var timeCounter = $("div#ui-datepicker-div .ui-timepicker-div .ui_tpicker_time").html();

                        if(arrMonitor.length < 2)
                            $(element).closest('div').find(".monitor-persian-date").html(timeCounter);
                        else
                            $(element).closest('div').find(".monitor-persian-date").html(arrMonitor[0]+' '+timeCounter);

                        //show timopicker by clicking out end

                        $("div#ui-datepicker-div .ui-timepicker-div").css('visibility', 'hidden');
                    }
                    //timepicker end

                    var data_gdate = $(element).attr('data-gdate'); //  2018/5/19

                    var data_jdate = $(element).attr('data-jdate');

                    $(element).removeAttr('data-jdate');
                    $(element).removeAttr('data-gdate');

                    try {
                        var data_gdate_array = data_gdate.split('/');

                        var income_year = data_gdate_array[0];
                        var income_month = data_gdate_array[1] - 1;
                        var income_day = data_gdate_array[2];


                        $(element).next('button').trigger('click');
                        //ui-state-default

                            $(document).find('#ui-datepicker-div').find("a.ui-state-default").each(function () {

                                    $(this).html(income_day);

                                    $(this).parent().attr('data-month', income_month);
                                    $(this).parent().attr('data-year', income_year);
                                    ...
                                    if (self.options.timePicker) {
                                        ...
                                        var sxsx = $(element).val();
                                        var auxAr = sxsx.split(/[ ]/);


                                        $(element).closest('div').find(".monitor-persian-date").html(data_jdate + ' ' + auxAr[1]);
                                    } else
                                        $(element).closest('div').find(".monitor-persian-date").html(data_jdate);
        ...
        persianDatepicker.prototype = {
            ...
            timepicker: function () {
                var self = this;
                _timepicker = $('<div style="height:110px;background-color:#f1f1f1"></div>');
                _timepicker.appendTo(this.calendar);
            },
            //timepicker end
            ...
            footer: function () {
                ...
                if (self.options.selectableMonths._indexOf(self.persianDate.month) > -1) {
                    ...
                    _resetbtn = $('<a class="goToday" />');

                    _resetbtn.attr("href", "javascript:;")
                        .html(' | ' + 'بازنشانی');

                    _resetbtn.bind("click", function () {
                        //console.log('sxsx!!!');
                        window.global_element_datepicker.attr('value', '');
                        window.global_element_datepicker.val('').change();
                        window.global_element_datepicker.closest('div').find(".monitor-persian-date").html('');
                    });


                    _resetbtn.appendTo(_footer);
                    //majidian end
